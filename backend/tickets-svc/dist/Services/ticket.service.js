...
class TicketService {
    ...
    async validarHabilidadesAgente(agenteId, empresaId) {
        try {
            const response = await axios.get(`${process.env.USUARIOS_SVC_URL}/usuarios/${agenteId}`, {
                headers: {
    ...
    async listarTickets(filtros = {}, options = {}) {
    ...
    async verificarAccesoChat(ticketId, usuarioId) {
        ...
        const tieneAcceso = (ticket.usuarioCreador?._id.toString() === usuarioId ||
    ...
    async actualizarClasificacion(ticketId, clasificacion) {
        ...
        if (clasificacion.tipo)
            ticket.tipo = clasificacion.tipo;
        if (clasificacion.prioridad)
            ticket.prioridad = clasificacion.prioridad;
        if (clasificacion.categoria)
            ticket.categoria = clasificacion.categoria;
        if (clasificacion.tiempoResolucion)
            ticket.tiempoResolucion = clasificacion.tiempoResolucion;
        if (clasificacion.tiempoRespuesta)
            ticket.tiempoRespuesta = clasificacion.tiempoRespuesta;
