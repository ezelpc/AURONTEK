...
const ticketController = {
    ...
    async listar(req, res) {
        try {
            ...
            if (typeof asignado !== 'undefined') {
                if (asignado === 'true')
                    filtros.agenteAsignado = { $ne: null };
                else if (asignado === 'false')
                    filtros.agenteAsignado = null;
            }
    ...
    async obtener(req, res) {
        try {
            ...
            const tieneAcceso = (req.usuario?.rol === 'admin-interno' ||
    ...
    async delegar(req, res) {
        try {
            ...
            const ticket = await ticketService.delegarTicket(id, becarioId, req.usuario.id, // El soporte actual se convierte en tutor
