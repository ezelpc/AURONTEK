...
router.patch('/:id/clasificacion', validateServiceToken, ticketController.actualizarClasificacion);
// ✅ NUEVO: Asignación automática por IA (solo IA service)
router.put('/:id/asignar-ia', validateServiceToken, ticketController.asignarIA);
// Actualizar estado (soporte, beca-soporte, admin-interno)
router.put('/:id/estado', authorize('soporte', 'beca-soporte', 'admin-interno'), ticketController.actualizarEstado);
// Asignar ticket manualmente (solo admin-interno)
router.put('/:id/asignar', authorize('admin-interno'), ticketController.asignar);
// ✅ Delegar ticket a becario (solo soporte)
router.put('/:id/delegar', authorize('soporte'), ticketController.delegar);
