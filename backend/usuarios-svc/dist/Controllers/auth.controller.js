...
const login = async (req, res) => {
    ...
    const { correo, email: emailAlt, contraseña, password: passwordAlt, captchaToken, recaptchaToken: recaptchaAlt, codigoAcceso, codigo_acceso: codigoAccesoAlt } = req.body;
    ...
    try {
        ...
        */
        ...
        if (esAdminGeneral) {
            ...
            usuarioEncontrado = await Admin_model_1.default.findOne({ correo: email.toLowerCase() });
            ...
            const passValido = await bcrypt_1.default.compare(password, usuarioEncontrado.contraseña);
            if (!passValido) {
            ...
            usuarioEncontrado = await usuario_service_1.default.encontrarUsuarioPorCorreo(email);
            if (!usuarioEncontrado) {
            ...
            const empresa = await empresa_service_1.default.encontrarEmpresaPorId(usuarioEncontrado.empresa);
            if (!empresa) {
        ...
        const token = (0, jwt_1.generarJWT)(payload);
...
const check = async (req, res) => {
    try {
        ...
        let usuario = null;
        if (esAdminGeneral) {
            usuario = await Admin_model_1.default.findById(id);
            if (!usuario) {
            ...
            usuario = await usuario_service_1.default.encontrarUsuarioPorId(id);
            if (!usuario) {
                return res.status(404).json({ msg: 'Usuario no encontrado' });
            }
...
exports.default = { login, register, logout, check };
